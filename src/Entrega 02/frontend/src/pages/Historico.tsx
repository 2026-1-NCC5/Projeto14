...
import { Calendar, Package, TrendingUp, Download, Scale } from "lucide-react";
...
const Historico = () => {
  ...
  // Recalcula os totais com base nos dados do banco
  const totalItems = sessions.reduce((s, r) => s + Number(r.items), 0);
  const pesoGeral = sessions.reduce((s, r) => s + Number(r.totalWeight), 0);
  const mediaItems = sessions.length > 0 ? Math.round(totalItems / sessions.length) : 0;
        ...
        <Card>
          <CardContent className="p-5 flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-foreground mt-1">{mediaItems}</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-left py-3 px-2 font-semibold text-foreground">Peso Total</th>
                ...
                {sessions.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-8 text-center text-muted-foreground">
                      Nenhuma contagem registrada ainda. Passe alimentos na IA para começar!
                    </td>
                  </tr>
                ) : (
                  sessions.map((s) => (
                    <tr key={s.id} className="border-b border-border last:border-0 hover:bg-secondary/50">
                      ...
                      <td className="py-3 px-2">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-foreground font-medium">{Number(s.totalWeight).toFixed(2)} kg</span>
                        </div>
                      </td>
                      <td className="py-3 px-2">
